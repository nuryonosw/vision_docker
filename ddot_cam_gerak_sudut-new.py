...
def init_cam_capture(nomor_cam,nama_dasar,ekst='jpg',nama_windows='frame'):
    ...
    grab.set(cv2.CAP_PROP_BUFFERSIZE, 1)
...
class MoveGroupPythonInterfaceTutorial(object):
    ...
    def go_to_joint_state(self):
        ...
        for ulang in range(4):
            ...
            move_group.go(joint_goal, wait=True)
            rospy.sleep(5)
            waktu_mulai_sekuen = datetime.datetime.now().strftime("%Y-%m-%d_%H%M%S")
            for j5 in range(11):
            
                print("Tilting joint5 pose ke",j5, "iterasi ke",ulang)
                ...
                cv2.imwrite('{}_{}.{}'.format(os.path.join(os.getcwd(), str(j5)),waktu_mulai_sekuen, 'jpg'), frame)
                n += 1
                grab.release()
                #print("============ Printing robot state")
                #print(robot.get_current_state())
